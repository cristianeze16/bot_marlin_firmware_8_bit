...
bool Running = true;
#if ENABLED(CAPAS)
unsigned long change_layer = 0;
unsigned long this_layer = 0;
unsigned long total_layer = 0;
#endif
bool filament_change_layer = false;
...
float soft_endstop_min[XYZ] = { X_MIN_BED, Y_MIN_BED, Z_MIN_POS },
  ...
  #endif
#endif
// static bool change_filament = false;
...
/*-------------------------------------------------------------------*/
void script_phase_1_z(){
  enqueue_and_echo_commands_P(PSTR(PHASE_1_Z));
 (void)drain_injected_commands_P();
}
void script_phase_2_z(){
   enqueue_and_echo_commands_P(PSTR(PHASE_2_Z));
  (void)drain_injected_commands_P();
}
void script_phase_z_end(){
   enqueue_and_echo_commands_P(PSTR(PHASE_END_Z));
  (void)drain_injected_commands_P();
}
/* -------------------------------------------------------------------*/
/**
...
inline void get_serial_commands() {
  ...
  while (commands_in_queue < BUFSIZE && (c = MYSERIAL0.read()) >= 0) {
    ...
    if (serial_char == '\n' || serial_char == '\r') {
      ...
      // if (serial_char == ';') serial_comment_mode = true; sin command mode
...
// inline void gcode_capas_total(){
//   char* p= parser.command_ptr;
// layer =NUMERIC(*p);
//    SERIAL_ECHOPAIR("Capa: ", layer);
//   // ok_to_send();
...
void process_parsed_command() {
  ...
  char*command_ptr = parser.command_ptr;
   if(*command_ptr == ';'){
     parser.codenum = 0;
        ++command_ptr;
        if(*command_ptr == 'L' ){
        ...
        if(*command_ptr == ':' ){
           ++command_ptr;
              do {
              parser.codenum *= 10, parser.codenum += *command_ptr - '0';
              //codenum *= 10, codenum += *p++ - '0';
               ++command_ptr;
              // } while (NUMERIC(*command_ptr)); // 
               } while (NUMERIC(*command_ptr)); // 
               SERIAL_ECHOLNPAIR("capas:",parser.codenum);
              this_layer = parser.codenum + 1;
              
           }else if(*command_ptr == '_'){
            ++command_ptr;
            ++command_ptr;
            ++command_ptr;
            ++command_ptr;
           ...
           do {
              ...
              SERIAL_ECHOLNPAIR("total capas:",parser.codenum);
                total_layer = parser.codenum;
            }             
           }
         }
        if(filament_change_layer){
          if(change_layer == this_layer){
