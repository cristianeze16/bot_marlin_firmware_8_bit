...
int eje;
bool flag_led;
int counting,anterior,contador;
// The main status screen
void lcd_status_screen();
// extern bool filament_change_layer;
...
uint16_t max_display_update_time = 0;
  ...
  void menu_iconos();
  void mover_ejes();
  void menu_icon_move_axis_distance();
  static void  menu_calentando_carga();
  static void regresar_chico();
  static void regresar_grande();
  void menu_temperature_preheat();
  void menu_preheat_manual();
  void menu_change_filament_bot();
  void menu_icon_move();
  void menu_icon_move_axis();
  void menu_icon_move_home();
  void volver_chico(int pos_encoder,int pos_x,screenFunc_t going_to);
  void volver_grande(int16_t pos_encoder,int pos_x,screenFunc_t going_to);
  void menu_choice_temp();
  void menu_bed_level();
  void calibracion_cama();
  void asistente_zonda();
  void confirmar_colision();
  void collision_z_alignment();
  FORCE_INLINE void barra(bool blink);
  void collisioning();
  void mensaje_leveling();
  void menu_guardar();
  void mensaje_zonda();
  void anim_barra(int waiting, int pos_x, const char * mensaje, screenFunc_t going_to);
  void guardar();
  void salir();
  void preheat_manual_hotend();
  void preheat_manual_heatbed();
  ...
  void manual_home_y();
  void manual_home_z();
  void eje_x_seleccionado();
  void eje_y_seleccionado();
  void eje_z_seleccionado();
...
void goto_menu_flow();
void goto_menu_preheat_manual_heatbed();
void goto_menu_preheat_manual_hotend();
 ...
 void error_capa();
  // void change_layer_menu();
  void lcd_move_menu();
  void lcd_control_menu();
  void lcd_control_temperature_menu();
  void lcd_control_motion_menu();
// iconos 
void icono_uno(int pos_encoder);
void icono_dos(int pos_encoder);
void icono_tres(int pos_encoder);
void icono_cuatro(int pos_encoder);
void icono_cinco(int pos_encoder);
void icono_seis(int pos_encoder);
void icono_siete(int pos_encoder);
void icono_ocho(int pos_encoder);
void icono_nueve(int pos_encoder);
void icono_diez(int pos_encoder);
...
void pre_fil_pla(){
 thermalManager.setTargetHotend(190, 0);
  lcd_goto_screen(menu_calentando_carga);
}
void pre_fil_abs(){
 thermalManager.setTargetHotend(230, 0);
...
// función iconos pequeños
void iconos_23p(int  ico_pos_x,int ico_pos_y,int ico_number,int pos_encoder,int x_mens, const char* nombre,const uint8_t* icono_a ,const uint8_t* icono_b,screenFunc_t display_goto){
  //  u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 9);
 if(ico_number == pos_encoder){
   u8g.setPrintPos(x_mens,63);
   u8g.setColorIndex(0);
   lcd_printPGM(nombre);
   u8g.setColorIndex(1); 
    u8g.drawBitmapP(ico_pos_x,ico_pos_y,ICONS_BYTES,ICONS_BITS,icono_b);
   if(use_click()){
     lcd_goto_screen(display_goto);
     }
    
 }else { u8g.drawBitmapP(ico_pos_x,ico_pos_y,ICONS_BYTES,ICONS_BITS,icono_a);}

 
}
  /*-------------------------------*/

// función iconos grandes
void iconos_31p(int  ico_pos_x,int ico_pos_y,int ico_number,int16_t pos_encoder,int x_mens, const char* nombre,const uint8_t* icono_a ,const uint8_t* icono_b,screenFunc_t display_goto){
  //  u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 9);
 if(ico_number == pos_encoder){
   u8g.setPrintPos(x_mens,63);
   u8g.setColorIndex(0);
   lcd_printPGM(nombre);
   u8g.setColorIndex(1); 
   u8g.drawBitmapP(ico_pos_x,ico_pos_y,EXPBYTE_WIDTH,EXP_HEIGHT,icono_b);
  ...
  }else {    u8g.drawBitmapP(ico_pos_x,ico_pos_y,EXPBYTE_WIDTH,EXP_HEIGHT,icono_a);}
...
void volver_grande(int16_t pos_encoder,int pos_x,screenFunc_t going_to){
   ...
   u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_BACK));
   u8g.setColorIndex(1); 
   u8g.drawBitmapP(pos_x,18,EXPBYTE_WIDTH,EXP_HEIGHT,volver_b);
    if(lcd_clicked){
     lcd_goto_screen(going_to);
     }
    
 }else {u8g.drawBitmapP(pos_x,18,EXPBYTE_WIDTH,EXP_HEIGHT,volver_a);
  }
   
}
  /*-------------------------------*/

void temp_volver_pred_manual(){

u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 11);
u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 11);
...
menu_numero = 3;
volver_grande(encoderPosition,9,regresar_grande);
iconos_31p(49,18,1,encoderPosition,21,PSTR(MSG_PREDET),temp_pred_a,temp_pred_b,menu_temperature_preheat);
iconos_31p(89,18,2,encoderPosition,21,PSTR(MSG_PREHEAT_CUSTOM),temp_manual_a,temp_manual_b,menu_preheat_manual);

}

void menu_temperature_preheat() { /* Filamento */
u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 11);
u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 11);
u8g.setPrintPos(20,8);
u8g.setColorIndex(0);
// lcd_put_u8str_P(50,63,PSTR(MSG_BACK));
lcd_printPGM(PSTR(MSG_PREDET));
u8g.setColorIndex(1);  

if(encoderPosition > 3 && encoderPosition <60){
 encoderPosition = 4;
}
...
volver_chico(encoderPosition,2,regresar_chico);
iconos_23p(27,23,1,encoderPosition,50,PSTR(MSG_PREHEAT_PLA), pla_preheat_a, pla_preheat_b,pre_pla);
iconos_23p(52,23,2,encoderPosition,50,PSTR(MSG_PREHEAT_ABS), abs_preh_a, abs_preh_b,pre_abs);
iconos_23p(77,23,3,encoderPosition,50,PSTR(MSG_PREHEAT_FLEX), flex_preh_a, flex_preh_b,pre_flex);
iconos_23p(102,23,4,encoderPosition,50,PSTR(MSG_PREHEAT_NLN), nln_a, nln_b,pre_nylon);

}

void preheat_filamento(){
...
menu_numero = 16;
volver_chico(encoderPosition,2,regresar_chico);
iconos_23p(27,23,1,encoderPosition,50,PSTR(MSG_PREHEAT_PLA), pla_preheat_a, pla_preheat_b,pre_fil_pla);
iconos_23p(52,23,2,encoderPosition,50,PSTR(MSG_PREHEAT_ABS), abs_preh_a, abs_preh_b,pre_fil_abs);
iconos_23p(77,23,3,encoderPosition,50,PSTR(MSG_PREHEAT_FLEX), flex_preh_a, flex_preh_b,pre_fil_flex);
iconos_23p(102,23,4,encoderPosition,50,PSTR(MSG_PREHEAT_NLN), nln_a, nln_b,pre_fil_nylon);
...
static void regresar_grande(){
 if(menu_numero == 0){
   lcd_goto_screen(menu_iconos,2);
 }else if(menu_numero == 1){
  lcd_goto_screen(menu_iconos,1);
 }else if(menu_numero == 2){
  lcd_goto_screen(menu_iconos,4);
 }else if(menu_numero == 3){
    if(card.cardOK){
     if(card.isFileOpen()){
      lcd_goto_screen(menu_printing,5);
     }else{
    lcd_goto_screen(menu_iconos,3);
     }
   }else{
   lcd_goto_screen(menu_iconos,3);
   }
 }else if(menu_numero == 4){
  lcd_goto_screen(menu_iconos,6);
 }else if(menu_numero == 5){
   lcd_goto_screen(menu_change_filament_bot,1);
 }else if(menu_numero == 6){
   lcd_goto_screen(menu_change_filament_bot,2);
 }else if(menu_numero == 40){
  lcd_goto_screen(menu_iconos,9);
 }else if(menu_numero == 45){
   lcd_goto_screen(menu_iconos,1);
 }
...
static void regresar_chico(){
 if(menu_numero == 3){
    lcd_goto_screen(menu_icon_move,1);
 }else if(menu_numero == 4){
   lcd_goto_screen(menu_icon_move_axis);
   }else if(menu_numero == 5){
     lcd_goto_screen(menu_icon_move_axis_distance,2);
   }else if(menu_numero == 6){
     lcd_goto_screen(menu_icon_move_axis_distance,3);
   }else if(menu_numero == 2){
     lcd_goto_screen(menu_icon_move_axis,2);
   }else if(menu_numero == 7){
    lcd_goto_screen(temp_volver_pred_manual,2);
   }else if(menu_numero == 8){
     lcd_goto_screen(temp_volver_pred_manual,1);
   }else if(menu_numero == 11 ){
    lcd_goto_screen(menu_icon_move_home,1);
   }else if(menu_numero == 12){
    lcd_goto_screen(menu_icon_move_home,2);
   }else if(menu_numero == 13){
   lcd_goto_screen(menu_icon_move_home,3);                       
  }else if(menu_numero == 14){
   lcd_goto_screen(menu_icon_move_home,4);                       
  }else if(menu_numero == 15){
    lcd_goto_screen(menu_iconos,4);
  }else if(menu_numero == 16){
   lcd_goto_screen(menu_iconos,2);
  }
}
...
void volver_chico(int pos_encoder,int pos_x,screenFunc_t going_to){
 if(pos_encoder == 0){
   ...
   lcd_printPGM(PSTR(MSG_BACK));
   u8g.setColorIndex(1); 
   u8g.drawBitmapP(pos_x,23,ICONS_BYTES,ICONS_BITS,volver_23_b);
    if(lcd_clicked){
     lcd_goto_screen(going_to);
     }
 } else { u8g.drawBitmapP(pos_x,23,ICONS_BYTES,ICONS_BITS,volver_23_a);}
}
...
/*-------------------------------*/
  ...
  void lcd_main_menu() {
    ...
    // MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
      ...
      if (card.cardOK) {
        if (card.isFileOpen()) {
          if (card.sdprinting)
          lcd_goto_screen(menu_printing);
        }else{
          lcd_goto_screen(menu_iconos);
        }
      }else{
          lcd_goto_screen(menu_iconos);
      }
   ...
   u8g.drawBox(0, 55, LCD_PIXEL_WIDTH,10);
    if(encoderPosition > 9 && encoderPosition <100){
      encoderPosition = 0;
    }
     if(encoderPosition > 200){
      ...
      icono_uno(encoderPosition); // volver status menu nunero 0
   icono_dos(encoderPosition); // menu sd menu nunero 1
   icono_tres(encoderPosition);// cambiar filamento menu nunero 2
   icono_cuatro(encoderPosition);//temperatura menu nunero 3
   icono_cinco(encoderPosition);// calibracion menu nunero 4
   icono_seis(encoderPosition);//luz led menu nunero 5
   icono_siete(encoderPosition);// mover ejes menu nunero 6
   icono_ocho(encoderPosition); // apagar motores menu nunero 7
   icono_nueve(encoderPosition);// aplicar laca menu nunero 8
   icono_diez(encoderPosition); // configuraciones menu nunero 9

   }

  /* Funcion de cada icono */
  /*-------------------------------*/
   void icono_uno(int pos_encoder) { // inicio
   if(pos_encoder == 0){
  
     u8g.drawBitmapP(2,3,ICONS_BYTES,ICONS_BITS,icon_main_b);
     u8g.setPrintPos(25,63);
     u8g.setColorIndex(0);
     lcd_printPGM(PSTR(MSG_MAIN));
     u8g.setColorIndex(1);  
       if(lcd_clicked){ 
      lcd_goto_screen(back_status);     
     }
    } else  { u8g.drawBitmapP(2,3,ICONS_BYTES,ICONS_BITS,icon_main_a);}
} 
   
  /*-------------------------------*/
  void icono_dos( int pos_encoder){ // menu sd

  if (pos_encoder == 1){
    if (card.cardOK) {
        if (!card.isFileOpen()) {
          u8g.drawBitmapP(27,3,ICONS_BYTES,ICONS_BITS,print_from_sd_b);
          u8g.setPrintPos(25,63);
          u8g.setColorIndex(0);         
          lcd_printPGM(PSTR(MSG_SD_INSERTED));    
          u8g.setColorIndex(1);
        } 
         if(lcd_clicked){
           lcd_goto_screen(lcd_sdcard_menu);
         }
        
    }  else {
       
          u8g.drawBitmapP(27,3,ICONS_BYTES,ICONS_BITS,no_sd_b);
          u8g.setPrintPos(10,63);
          u8g.setColorIndex(0);
         lcd_printPGM(PSTR(MSG_NO_CARD));    
          u8g.setColorIndex(1);
        if(lcd_clicked){enqueue_and_echo_commands_P(PSTR("M21"));}
    }
       } else{         
        if (card.cardOK){
          u8g.drawBitmapP(27,3,ICONS_BYTES,ICONS_BITS,print_from_sd_a);
        }else{
          u8g.drawBitmapP(27,3,ICONS_BYTES,ICONS_BITS,no_sd_a);
        }
      }
  }
  /*-------------------------------*/
   void icono_tres( int pos_encoder){ // cambiar filamento
    if( pos_encoder == 2){
   u8g.drawBitmapP(52,3,ICONS_BYTES,ICONS_BITS,change_filament_b);
   u8g.setPrintPos(14,63);

   u8g.setColorIndex(0);         
   lcd_printPGM(PSTR(MSG_FILAMENTCHANGE));     
   u8g.setColorIndex(1); 
     if(lcd_clicked){
    lcd_goto_screen(menu_choice_temp);     
     }
    } else  {u8g.drawBitmapP(52,3,ICONS_BYTES,ICONS_BITS,change_filament_a);} 
   }
  /*-------------------------------*/
   void icono_cuatro (int pos_encoder){ // menu temperatura
    if(pos_encoder == 3){
   u8g.drawBitmapP(77,3,ICONS_BYTES,ICONS_BITS,icon_temperature_b);
   if(lcd_clicked){ lcd_goto_screen(temp_volver_pred_manual);}
   u8g.setPrintPos(30,63);
   u8g.setColorIndex(0);      
   lcd_printPGM(PSTR(MSG_TEMPERATURE));       
   u8g.setColorIndex(1); 
    }else {u8g.drawBitmapP(77,3,ICONS_BYTES,ICONS_BITS,icon_temperature_a);}
   }

  /*-------------------------------*/
   void icono_cinco(int pos_encoder) {// calibracion
    if( pos_encoder == 4 ){
   u8g.drawBitmapP(102,3,ICONS_BYTES,ICONS_BITS,calibracion_b);
   u8g.setPrintPos(30,63);

   u8g.setColorIndex(0);         
  lcd_printPGM(PSTR(MSG_LEVEL_BED));    
   u8g.setColorIndex(1);
   if(lcd_clicked){lcd_goto_screen(menu_bed_level);}     
    } else  {u8g.drawBitmapP(102,3,ICONS_BYTES,ICONS_BITS,calibracion_a);}
   }
  /*-------------------------------*/
  ...
  void menu_bed_level(){
   ...
   u8g.setPrintPos(20,8);
   u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_CALIBRATIONS));
   u8g.setColorIndex(1); 
   ...
   menu_numero = 15;
   volver_chico(encoderPosition,15,regresar_chico);
   iconos_23p(40,23,1,encoderPosition,30,PSTR(MSG_LEVEL_BED), calibracion_a,calibracion_b,calibracion_cama);
   iconos_23p(65,23,2,encoderPosition,12,PSTR(MSG_PROBE_WIZARD), zoffset_a, zoffset_b,asistente_zonda);
   iconos_23p(90,23,3,encoderPosition,17,PSTR(MSG_AUTO_Z_ALIGN), zaling_a,zaling_b,confirmar_colision);
...
void confirmar_colision(){
 ...
 u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 11);
  ...
  lcd_printPGM(PSTR(WARNING_COLISION_1));
  u8g.setPrintPos(5,30);
  lcd_printPGM(PSTR(WARNING_COLISION_2));
  u8g.setPrintPos(5,40);
  lcd_printPGM(PSTR(WARNING_COLISION_3));
  u8g.setPrintPos(5,50);
  lcd_printPGM(PSTR(MSG_ZOFFSET_CALIBRATION_WIZ_6));
  

 if(lcd_clicked){
 lcd_goto_screen(collision_z_alignment); 
  }
} 
  /*-------------------------------*/

void collision_z_alignment(){// alineacion
  bool blink = lcd_blink();
   lcdDrawUpdate = LCDVIEW_REDRAW_NOW;
  defer_return_to_status = true;
u8g.drawBox(0,0,LCD_PIXEL_WIDTH, 11);
u8g.drawBox(0,54,LCD_PIXEL_WIDTH, 14);
  u8g.setPrintPos(10,8);
u8g.setColorIndex(0);
lcd_printPGM(PSTR(MSG_BED_HOMING_WAIT)); 
// lcd_put_u8str_P(10,63,PSTR(MSG_LEVEL_BED_HOMING)); 
u8g.setColorIndex(1);
barra(blink);
script_phase_1_z();
lcd_goto_screen(collisioning);
//  enqueue_and_echo_commands_now_P(PSTR("G1 Z150 F1000\nM400"));
...
barra(blink);
 if(lcd_blink()){
   anterior = counting;
 }else if(anterior == counting && !lcd_blink()/* && all_axes_known() && !is_busy*/){
  counting++;
  anterior = 0;
 }  
if( lcdDrawUpdate == 1 && counting > 0 && counting < limit){
// if( lcd_blink() && counting > 0 && counting < limit){

 script_phase_2_z();
}else if(lcdDrawUpdate == 2 && counting > 0 && counting < limit)/*&& queue.enqueue_one_P)*/{
// }else if(!lcd_blink() && counting > 0 && counting < limit)/*&& queue.enqueue_one_P)*/{
  script_phase_2_z();
...
void calibracion_cama(){
 ...
 lcd_goto_screen(mensaje_leveling);
}
void mensaje_leveling(){
//  void anim_barra(int waiting, int pos_x, const char * mensaje, screenFunc_t going_to)
 anim_barra(1500,30,PSTR(MSG_BED_LEVELING),menu_guardar);
}
void asistente_zonda(){
 enqueue_and_echo_commands_now_P(PSTR("G28\nG1 Z0 F1000"));
lcd_goto_screen(mensaje_zonda);   

}
void mensaje_zonda(){
  // ui.anim_barra_carga_descarga(0,300,20,PSTR(MSG_MOVING),lcd_babystep_zoffset);
  anim_barra(500,20,PSTR(MSG_MOVING),lcd_babystep_zoffset);
}
  /*-------------------------------*/
...
// lcd_put_u8str_P(25,63,PSTR(MSG_SAVE));
lcd_printPGM(PSTR(MSG_QUESTIONSAVE));
u8g.setColorIndex(1);  
...
// menu_numero = 0;
// volver_grande(encoderPosition,9,regresar_grande);
iconos_31p(23,18,0,encoderPosition,40,PSTR(MSG_SAVE),guardar_a,guardar_b,guardar);
iconos_31p(74,18,1,encoderPosition,16,PSTR(MSG_EXIT),salir_a,salir_b,salir);
}
...
void menu_preheat_manual() {
   ...
   u8g.setColorIndex(0);
u8g.setPrintPos(20,8);
  //  lcd_put_u8str_P(40,63,PSTR(MSG_BACK));
  lcd_printPGM(PSTR(MSG_PREHEAT_CUSTOM));
   u8g.setColorIndex(1); 
 if (encoderPosition >4 && encoderPosition <50){
    encoderPosition = 4;
 }
 if(encoderPosition > 50){
   encoderPosition = 0;
  ...
  menu_numero = 7;
   volver_chico(encoderPosition,2,regresar_chico);
   iconos_23p(27,23,1,encoderPosition,40,PSTR(MSG_PREHEAT_NOZZLE), calibracion_a,calibracion_b,goto_menu_preheat_manual_hotend);
   iconos_23p(52,23,2,encoderPosition,45,PSTR(MSG_PREHEAT_HEATBED), heat_bed_a, heat_bed_b,goto_menu_preheat_manual_heatbed);
   iconos_23p(77,23,3,encoderPosition,30,PSTR(MSG_FAN_SPEED), fan_manual_a,fan_manual_b,manual_fan_speed);
   iconos_23p(102,23,4,encoderPosition,30,PSTR(MSG_COOLDOWN), cooldown_a,cooldown_b,cooldown);
   
 }
void goto_menu_preheat_manual_hotend(){
  lcd_goto_screen(preheat_manual_hotend, thermalManager.degTargetHotend(active_extruder));
}

void goto_menu_preheat_manual_heatbed(){
 lcd_goto_screen(preheat_manual_heatbed,thermalManager.degTargetBed());
}

void preheat_manual_heatbed() {
encoderRateMultiplierEnabled = true;
  if (encoderPosition >= BED_MAXTEMP && encoderPosition  <= 400){
  encoderPosition = BED_MAXTEMP;
   u8g.setPrintPos(64, 37);
      lcd_print(itostr3(int(encoderPosition)));
  }
  else if ( encoderPosition >= 0 &&  encoderPosition <= BED_MAXTEMP   ){
    u8g.setPrintPos(64, 37);
      lcd_print(itostr3(int(encoderPosition)));
...
u8g.setPrintPos(50,63);
   ...
   u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_PREHEAT_HEATBED));
   u8g.setPrintPos(20,8);
    lcd_printPGM(PSTR(MSG_PREHEAT_CUSTOM));
   u8g.setColorIndex(1); 
  ...
  //  EDIT_ITEM_FAST(int3, MSG_BED, &thermalManager.temp_bed.target, 0, BED_MAX_TARGET, thermalManager.start_watching_bed);
     if(lcd_clicked){
       thermalManager.setTargetBed(encoderPosition);
 ...
 encoderRateMultiplierEnabled = true;
  ...
  // encoderPosition = encoderPosition & 0xFFFF;
  // encoderPosition = encoderPosition + thermalManager.degTargetHotend(0);

  if (encoderPosition >= 0 && encoderPosition <= HEATER_0_MAXTEMP){
    u8g.setPrintPos(64, 37);
  lcd_print(itostr3(int(encoderPosition)));
  
  }else if (encoderPosition >= HEATER_0_MAXTEMP &&  encoderPosition <= 560  ){
     u8g.setPrintPos(64, 37);
     encoderPosition = HEATER_0_MAXTEMP;
     lcd_print(itostr3(int(encoderPosition)));
  ...
  lcd_print(itostr3(int(encoderPosition)));
   ...
   u8g.setColorIndex(0);
    ...
    lcd_printPGM(PSTR(MSG_PREHEAT_NOZZLE));
     u8g.setPrintPos(20, 8);
    lcd_printPGM(PSTR(MSG_PREHEAT_CUSTOM));
   u8g.setColorIndex(1); 
     u8g.setPrintPos(10, 37);
      lcd_printPGM(PSTR(MSG_HEAT_NOZZLE));
   
  
  if(lcd_clicked){
thermalManager.setTargetHotend(encoderPosition,0);
lcd_return_to_status();
     }
...
encoderRateMultiplierEnabled = true;
  ...
  if(encoderPosition >= 0 && encoderPosition <= 255) {
      u8g.setPrintPos(91, 44);
      lcd_print(itostr3(int(encoderPosition)));
    }else if(encoderPosition >= 250 && encoderPosition <= 500){
      encoderPosition = 255;
      u8g.setPrintPos(91, 44);
      lcd_print(itostr3(int(encoderPosition)));
    }else{
...
u8g.setColorIndex(0);
lcd_printPGM(PSTR(MSG_BED_HOMING_WAIT)); 
u8g.setPrintPos(pos_x,63);
lcd_printPGM(mensaje);
// lcd_put_u8str_P(pos_x,63,mensaje); 
u8g.setColorIndex(1);
barra(blink);
contador ++;
 if(contador > waiting){
   contador = 0;
  //  ui.defer_return_to_status = false;
  lcd_goto_screen(going_to);
...
void quitar_filamento(){
if (contador == 0){
  enqueue_and_echo_commands_P(PSTR("G91\nG1 E15 F150\nG90\nG91\nG1 E-100 F300\nG90\nM84"));
}
menu_numero = 6;
anim_barra(520,7,PSTR(MSG_FILAMENT_UNLOADING),regresar_grande);
...
enqueue_and_echo_commands_P(PSTR("G91\nG1 E100 F150\nG90\nM84"));
 ...
 menu_numero = 5;
anim_barra(700,10,PSTR(MSG_FILAMENT_LOADING),regresar_grande);
...
menu_numero = 0;
volver_grande(encoderPosition,9,regresar_grande);
iconos_31p(49,18,1,encoderPosition,16, PSTR(MSG_FILAMENTLOAD),cargar_fil_a,cargar_fil_b,cargar_filamento);
iconos_31p(89,18,2,encoderPosition,16, PSTR(MSG_FILAMENTUNLOAD),desc_a,desc_b,quitar_filamento);

}
  /*-------------------------------*/

/* Menu seleccion  temperatura para carga de filamento*/
void menu_choice_temp() {
 defer_return_to_status = true;
  // encoderPosition = encoderPosition & 0xFFFF;
 if (encoderPosition >= HEATER_0_MAXTEMP ){
   encoderPosition = HEATER_0_MAXTEMP;
    // encoderPosition = HEATER_0_MAXTEMP - (HOTEND_OVERSHOOT);
  }
if (encoderPosition <= 0){
  ...
  encoderRateMultiplierEnabled = true;
   u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 20);
   u8g.setColorIndex(0);
   u8g.setPrintPos(33,8); 
   lcd_printPGM(PSTR(MSG_MENSAJE_1));
   u8g.setPrintPos(21,17); 
   lcd_printPGM(PSTR(MSG_MENSAJE_2)); 
   u8g.setColorIndex(1); 
   u8g.setPrintPos(24,44); 
   lcd_printPGM(PSTR(MSG_MENSAJE_3));
   u8g.setPrintPos(84, 44);
   lcd_print(itostr3(int(encoderPosition)));

   
    if (lcd_clicked) {
     if(encoderPosition <= 110){
     lcd_goto_screen(preheat_filamento);
     
     }else{
    thermalManager.setTargetHotend(encoderPosition,1);
   lcd_goto_screen(menu_calentando_carga);
     }
    }
...
u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 11);
u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 11);
u8g.setPrintPos(14,8); 
u8g.setColorIndex(0);
lcd_printPGM(PSTR(MSG_BED_HOMING_WAIT));
u8g.setPrintPos(23,63); 
lcd_printPGM(PSTR(MSG_HEATING));
u8g.setColorIndex(1);
barra(blink);
u8g.setPrintPos(70,28);
lcd_print(itostr3(int(thermalManager.degHotend(0)) + 0.5 ));
lcd_print('/');
lcd_print(itostr3(int(thermalManager.degTargetHotend(0)) + 0.5 ));
 if(thermalManager.degHotend(active_extruder) >= thermalManager.degTargetHotend(active_extruder)- 3){
  defer_return_to_status = true;
 lcd_goto_screen(menu_change_filament_bot);
}
...
void apagar_stepper(){
     ...
     enqueue_and_echo_commands_P(PSTR("M117 Motores apagados "));
     lcd_return_to_status();

}
void continuar_print(){
  lcd_sdcard_resume();
 lcd_goto_screen(menu_printing,5);
}
void pausar_print(){  
...
void cambiar_fila(){
  ...
  /*-------------------------------*/
   ...
   void icono_seis_off(int pos_encoder){ // control led
     if(pos_encoder == 5){
    // case 5:
  //  u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 9);
   u8g.drawBitmapP(2,30,ICONS_BYTES,ICONS_BITS,led_control_b_on);
   u8g.setPrintPos(30,63);
   u8g.setColorIndex(0);
   ...
   u8g.setColorIndex(1); 
    ...
    if(lcd_clicked){ 
      lcd_goto_screen(toggle_luz_led);
      }
    }else {u8g.drawBitmapP(2,30,ICONS_BYTES,ICONS_BITS,led_control_a_on);}
   }
  /*-------------------------------*/

 void icono_seis_on(int pos_encoder){
       if(pos_encoder == 5){
 
   u8g.drawBitmapP(2,30,ICONS_BYTES,ICONS_BITS,led_control_b);
   u8g.setPrintPos(30,63);
   u8g.setColorIndex(0);
  ...
  if(lcd_clicked){  
    ...
    }else {u8g.drawBitmapP(2,30,ICONS_BYTES,ICONS_BITS,led_control_a);}
   }
  /*-------------------------------*/
    void icono_seis(int pos_encoder){
    if(case_light_on){
      icono_seis_on(pos_encoder);
    }else{
      icono_seis_off(pos_encoder);
      }
    }
  /*-------------------------------*/
  void toggle_luz_led(){
  const bool busy = IS_SD_PRINTING();
  ...
  //  lcd_goto_screen(menu_main);
   if(busy){
  lcd_goto_screen(menu_printing,4);

  //  encoderPosition = 4;      
   }else{
   lcd_goto_screen(menu_iconos,5);
...
u8g.setPrintPos(35,8);
u8g.setColorIndex(0);
lcd_printPGM(PSTR(MSG_MOVE_AXIS));
u8g.setColorIndex(1);  

if(encoderPosition >1 && encoderPosition < 40){
  encoderPosition = 2;
...
menu_numero = 4;
volver_grande(encoderPosition,8,regresar_grande);
iconos_31p(49,17,1,encoderPosition,33,PSTR(MSG_MOVE_AXIS),mover_a,mover_b,menu_icon_move_axis);
iconos_31p(89,17,2,encoderPosition,20,PSTR(MSG_AUTO_HOME),home_a,home_b,menu_icon_move_home);
...
u8g.setPrintPos(20,8);
u8g.setColorIndex(0);
lcd_printPGM(PSTR(MSG_AUTO_HOME));
u8g.setColorIndex(1);  

if(encoderPosition> 4 && encoderPosition< 40){
  encoderPosition= 4;
...
menu_numero = 3;
volver_chico(encoderPosition,2,regresar_chico);
iconos_23p(27,23,1,encoderPosition,20,PSTR(MSG_AUTO_HOME),icon_main_a,icon_main_b,manual_home);
iconos_23p(52,23,2,encoderPosition,40,PSTR(MSG_MOVE_X),move_x_a,move_x_b,manual_home_x);
iconos_23p(77,23,3,encoderPosition,40,PSTR(MSG_MOVE_Y),move_y_a,move_y_b,manual_home_y);
iconos_23p(102,23,4,encoderPosition,40,PSTR(MSG_MOVE_Z),move_z_a,move_z_b,manual_home_z);
...
if(encoderPosition > 2 && encoderPosition < 40){
  encoderPosition = 3;
...
menu_numero = 3;
volver_chico(encoderPosition,15,regresar_chico);
iconos_23p(40,23,1,encoderPosition,48,PSTR(MSG_MOVE_X),move_x_a,move_x_b,eje_x_seleccionado);
iconos_23p(65,23,2,encoderPosition,48,PSTR(MSG_MOVE_Y),move_y_a,move_y_b,eje_y_seleccionado);
iconos_23p(90,23,3,encoderPosition,48,PSTR(MSG_MOVE_Z),move_z_a,move_z_b,eje_z_seleccionado);
...
void menu_icon_move_axis_distance() {
 ...
 if(encoderPosition > 60){
  ...
  //  ui.save_previous_screen();
   volver_chico(encoderPosition,15,regresar_chico);
   iconos_23p(40,23,1,encoderPosition,31,PSTR(MSG_MOVE_01MM),move_01_a,move_01_b,move_eje_01);
   iconos_23p(65,23,2,encoderPosition,31,PSTR(MSG_MOVE_1MM),move_1_a,move_1_b,move_eje_1);
   iconos_23p(90,23,3,encoderPosition,31,PSTR(MSG_MOVE_10MM),move_10_a,move_10_b,move_eje_10);
 

}

void move_eje_01(){
  // _goto_manual_move(0.1f);
  // menu_numero = 4;
  move_menu_scale = 0.1;
  mover_ejes();
...
void move_eje_10(){
  // _goto_manual_move(10);
  // menu_numero = 6;
  move_menu_scale = 10.0;
  mover_ejes();
}

void menu_filament_change_printing(){
 
   u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 10);
   u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 10);
  ...
  menu_numero = 2;
  // volver_grande(encoderPosition,9,back_status);
  // change_now(encoderPosition);
  // change_layer_menu(encoderPosition);
volver_grande(encoderPosition,9,regresar_grande);
iconos_31p(49,18,1,encoderPosition,49, PSTR(MSG_FILAMENTCHANGE_NOW),cargar_fil_a,cargar_fil_b,cambiar_fila);
iconos_31p(89,18,2,encoderPosition,40, PSTR(MSG_FILAMENTCHANGE_LAYER),desc_a,desc_b,layer_selection);
...
//     u8g.setColorIndex(0);
//     lcd_printPGM(PSTR(MSG_FILAMENTCHANGE_NOW));//5
//     u8g.setColorIndex(1);
//     u8g.drawBitmapP(49,18,4,30,cargar_fil_b);
//     if(use_click){
//       lcd_goto_screen(cambiar_fila);
//     }
//  }else {
//        u8g.drawBitmapP(49,18,4,30, cargar_fil_a);
...
void error_capa(){
  ...
  u8g.setColorIndex(0);
  lcd_printPGM(PSTR("Error capas"));
  u8g.setPrintPos(19, 39);
  lcd_printPGM(PSTR("no establecidas"));
 if (lcd_clicked) {
    lcd_goto_screen(back_status);
  }
...
//     u8g.setColorIndex(1);
//     u8g.drawBitmapP(89,18,4,30,desc_b);
//     if(use_click){
//        lcd_goto_screen(layer_selection);
//       }else if(total_layer == 0 || total_layer <= this_layer){
//         lcd_goto_screen(error_capa);
      
//       }
    
// }else{
//       u8g.drawBitmapP(89,18,4,30, desc_a);
// }
// }
void layer_selection(){
if(total_layer == 0 || total_layer <= this_layer){
lcd_goto_screen(error_capa);
}
u8g.drawBox(0, 0, 128, 18);
  u8g.setPrintPos(31, 8);
  u8g.setColorIndex(0);
  lcd_printPGM(PSTR("Seleccionar"));
  u8g.setPrintPos(22, 16);
  lcd_printPGM(PSTR("capa de cambio"));
  ...
  if(encoderPosition >= this_layer + 1 && encoderPosition <= total_layer) {
  }else if(encoderPosition >= total_layer && encoderPosition <= this_layer + 1000){
    encoderPosition = total_layer;
  }else{
    encoderPosition = this_layer + 1;
  }
  lcd_print(ftostr5rj(encoderPosition));
  if(lcd_clicked) {
    change_layer = encoderPosition;
    filament_change_layer = true;
    lcd_goto_screen(back_status);
  }
...
void menu_printing(){
   ...
   u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 10);
  if(encoderPosition > 7 && encoderPosition < 80){
   encoderPosition = 8;
      }
  if(encoderPosition >80){
    encoderPosition = 0;
  }
/*int16_t posicion_encoder = encoderPosition & 0xFFFF;*/

// void iconos_23p(int  ico_pos_x,int ico_pos_y,int ico_number,int16_t pos_encoder,int x_mens, const char* nombre,const uint8_t* icono_a ,const uint8_t* icono_b,screenFunc_t display_goto){
  iconos_23p(2,3,0,encoderPosition,25,PSTR(MSG_MAIN),icon_main_a,icon_main_b,back_status);
  iconos_23p(27,3,1,encoderPosition,30,PSTR(MSG_TEMPERATURE),icon_temperature_a,icon_temperature_b,temp_volver_pred_manual);
  iconos_23p(52,3,2,encoderPosition,30,PSTR(MSG_BABYSTEP_Z),calibracion_a,calibracion_b,lcd_babystep_zoffset);
  iconos_23p(77,3,3,encoderPosition,30,PSTR(MSG_FILAMENTCHANGE),change_filament_a,change_filament_b,menu_filament_change_printing);
if(case_light_on){
 iconos_23p(102,3,4,encoderPosition,35,PSTR(MSG_LED_OFF),led_control_a,led_control_b,toggle_luz_led);
 
}else{ 
 
 iconos_23p(102,3,4,encoderPosition,30,PSTR(MSG_LED_ON),led_control_a_on,led_control_b_on,toggle_luz_led);
   } 
  //   if (TERN0(MACHINE_CAN_PAUSE, printingIsPaused()))
  //     ACTION_ITEM(MSG_RESUME_PRINT, ui.resume_print);
  if(card.sdprinting){
  iconos_23p(2,30,5,encoderPosition,10,PSTR(MSG_PAUSE_PRINT),pause_a,pause_b,pausar_print);
  }else{
  iconos_23p(2,30,5,encoderPosition,10,PSTR(MSG_PAUSE_PRINT),play_a,play_b, continuar_print);
  }


  iconos_23p(27,30,6,encoderPosition,25,PSTR(MSG_FLOW),icon_config_a,icon_config_b,goto_menu_flow);
  iconos_23p(52,30,7,encoderPosition,30,PSTR(MSG_VELOCITY),velocidad_a,velocidad_b,go_speed);
  iconos_23p(77,30,8,encoderPosition,30,PSTR(MSG_STOP_PRINT),cancelar_a,cancelar_b,parar_print);
}
void goto_menu_flow(){
 lcd_goto_screen(menu_flow,planner.flow_percentage[active_extruder]);
...
void menu_flow(){
   ...
   u8g.setPrintPos(33,8);
    ...
    lcd_printPGM(PSTR(MSG_SELECT));
   u8g.setPrintPos(50,17);
    lcd_printPGM(PSTR(MSG_FLOW));
    u8g.setColorIndex(1);
   u8g.setPrintPos(24,44);
    lcd_printPGM(PSTR(MSG_FLOW));
    
    // encoderPosition = planner.flow_percentage[active_extruder] + encoderPosition; 
    u8g.setPrintPos(84, 44);
    lcd_print(itostr3(int(encoderPosition)));


  if(use_click()){
    planner.flow_percentage[active_extruder] = encoderPosition;      
    planner.refresh_e_factor(active_extruder);
   lcd_goto_screen(menu_printing,6);
      }
  }
...
void go_speed(){
lcd_goto_screen(menu_speed,feedrate_percentage);
...
void menu_speed() {
 encoderRateMultiplierEnabled = true;
   ...
   u8g.drawBox(0, 0, 128, 20);
    ...
    u8g.setColorIndex(0);
    lcd_printPGM(PSTR(MSG_SELECT));
    u8g.setPrintPos(38, 17);
     lcd_printPGM(PSTR(MSG_SPEED));
   ...
   if(encoderPosition >= 0 && encoderPosition <= 300) {
      u8g.setPrintPos(84, 44);
      lcd_print(itostr3(int(encoderPosition)));
    }else if(encoderPosition >= 300 && encoderPosition <= 600){
      encoderPosition = 300;
      u8g.setPrintPos(84, 44);
      lcd_print(itostr3(int(encoderPosition)));
    } else{
...
void manual_home(){
 ...
 lcd_goto_screen(regresar_chico);
}
...
void manual_home_z(){
 ...
 lcd_goto_screen(regresar_chico);
  ...
  /*-------------------------------*/
  void icono_siete(int pos_encoder){ // mover ejes
    if(pos_encoder == 6){
   u8g.drawBitmapP(27,30,ICONS_BYTES,ICONS_BITS,icon_move_b);
   u8g.setPrintPos(35,63);
   u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_MOVE_AXIS));
    u8g.setColorIndex(1);      
      if(use_click()){ lcd_goto_screen(menu_icon_move);
     }
  } else  { u8g.drawBitmapP(27,30,ICONS_BYTES,ICONS_BITS,icon_move_a);}
}
/*-------------------------------*/
  /*-------------------------------*/
  void icono_ocho(int pos_encoder){ // apagar motores
      // case 7:
    if(pos_encoder == 7){
    // u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 9);
    u8g.drawBitmapP(52,30,ICONS_BYTES,ICONS_BITS,motor_off_b);
   u8g.setPrintPos(25,63);
    u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_DISABLE_STEPPERS));
    u8g.setColorIndex(1);
  if(use_click()){lcd_goto_screen(apagar_stepper);
     }
    }else{u8g.drawBitmapP(52,30,ICONS_BYTES,ICONS_BITS,motor_off_a);}
}
  /*-------------------------------*/

 FORCE_INLINE void barra(bool blink){
  if(lcd_blink()){
 u8g.drawBitmapP(13,30,13,7,barra_espera_1);
 }else{
 u8g.drawBitmapP(13,30,13,7,barra_espera_2);
 }
...
void mov_laca(){
anim_barra(500,10,PSTR(MSG_MOVING),presione_click);
...
void presione_click(){
 ...
 u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 11);
  ...
  u8g.setPrintPos(5,30);
  lcd_printPGM(PSTR(MSG_ADHESIVE_1));
  u8g.setPrintPos(5,40);

  lcd_printPGM(PSTR(MSG_ADHESIVE_2));
 if(use_click()){ 
      enqueue_and_echo_commands_P(PSTR("G28"));
  lcd_goto_screen(menu_iconos,8); 
  }
}      
  /*-------------------------------*/

 void icono_nueve(int pos_encoder){  // menu aplicar laca
  // bool blink = ui.get_blink();
 if( pos_encoder == 8){          
   u8g.drawBitmapP(77,30,ICONS_BYTES,ICONS_BITS,aplicar_laca_b);
  u8g.setPrintPos(30,63);
   u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_ADHESION));
   u8g.setColorIndex(1);
    // barra(blink);
    if(use_click()){     
      enqueue_and_echo_commands_P(PSTR("M203 Z10\nG28\nG1 Z150 F10000\nG1 Y200 F10000\nM203 Z5"));
      lcd_goto_screen(mov_laca);
      //  lcd_goto_screen(anim_barra_laca);  
   } 
 }  else { u8g.drawBitmapP(77,30,ICONS_BYTES,ICONS_BITS,aplicar_laca_a);}
  ...
  if( pos_encoder == 9){
    // case 9:
  //  u8g.drawBox(0, 55, LCD_PIXEL_WIDTH, 9);
   u8g.drawBitmapP(102,30,ICONS_BYTES,ICONS_BITS,icon_config_b);
  u8g.setPrintPos(22,63);
   u8g.setColorIndex(0);
   lcd_printPGM(PSTR(MSG_CONTROL));
   u8g.setColorIndex(1);  
    if(use_click()){  lcd_goto_screen(lcd_control_menu);
     }
    }else{  u8g.drawBitmapP(102,30,ICONS_BYTES,ICONS_BITS,icon_config_a);}
   }
  /*-------------------------------*/
   ...
   */
    ...
    void _lcd_babystep(const AxisEnum axis, const char* msg) {
      ...
      void lcd_babystep_zoffset() {
         if (use_click()) { lcd_goto_screen(menu_guardar); }
        defer_return_to_status = true;
        // if (use_click()) { return lcd_goto_previous_menu_no_defer(); }
        // defer_return_to_status = true;
        ENCODER_DIRECTION_NORMAL();
