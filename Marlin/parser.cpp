...
void GCodeParser::parse(char *p) {
  ...
  // Bail if the letter is not G, M, or T
   switch (letter) { case ';': case 'G': case 'M': case 'T': break; default: return; }
  ...
  while (const char code = *p++) {                    // Get the next parameter. A NUL ends the loop
    ...
    if (!WITHIN(*p, 'A', 'Z')) {                // Another parameter right away?
      ...
      //     //  SERIAL_ECHO("capa3:");   
      //         do {
      //         codenum *= 10, codenum += *command_ptr - '0';
      //         ++command_ptr;
      //         } while (NUMERIC(*command_ptr)); // 
              
      //         this_layer = codenum;
      //         gcode_M720(this_layer);
      ...
      //         ++command_ptr;
      //         } while (NUMERIC(*command_ptr)); // 
      //         SERIAL_ECHOLNPAIR("total capas:",codenum);
      //           total_layer = codenum;
      //           gcode_M721(total_layer);
