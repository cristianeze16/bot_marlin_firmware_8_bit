...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_CONTAINS(XYZ_FRAME_TOP, XYZ_FRAME_TOP + XYZ_FRAME_HEIGHT - 1)) {
    ...
    if (PAGE_CONTAINS(XYZ_BASELINE - (INFO_FONT_HEIGHT - 1), XYZ_BASELINE)) {
      ...
      lcd_printPGM(PSTR(MSG_LAYER));
      /* capa / capa total */
      int pos_layer = 15;

      if(this_layer >= 0 && this_layer < 10){
      u8g.setPrintPos(pos_layer, XYZ_BASELINE);
      }else if(this_layer >= 10 && this_layer < 100){
      u8g.setPrintPos(pos_layer + 2, XYZ_BASELINE);
      }else if(this_layer >= 100 && this_layer <= 1000){
      u8g.setPrintPos(pos_layer + 10, XYZ_BASELINE);
      }else if(this_layer >= 1000 && this_layer <= 10000){
      u8g.setPrintPos(pos_layer + 11, XYZ_BASELINE);
      }else{
      u8g.setPrintPos(pos_layer + 14, XYZ_BASELINE);
      }
      lcd_print(ftostr5rj(this_layer));
      lcd_print('/');
      if(total_layer >= 0 && total_layer < 10){
      u8g.setPrintPos(pos_layer + 12, XYZ_BASELINE);
      }else if(total_layer >= 10 && total_layer < 100){
      u8g.setPrintPos(pos_layer + 14 , XYZ_BASELINE);
      }else if(total_layer >= 100 && total_layer < 1000 ){
      u8g.setPrintPos(pos_layer + 33 , XYZ_BASELINE);
      }else if(total_layer >= 1000 && total_layer <10000){
      u8g.setPrintPos(pos_layer + 38 , XYZ_BASELINE);
      }else{
      u8g.setPrintPos(pos_layer + 40 , XYZ_BASELINE);
      }
      lcd_print(ftostr5rj(total_layer));
